...
class PatchPlaneApproxLoss(nn.Module):
    def __init__(self, config):
        ...
        # K_np = torch.Tensor([[fx_d, 0, cx_d, 0],
        #                  [0, fy_d, cy_d, 0],
        #                  [0, 0, 1, 0],
        #                  [0, 0, 0, 1]])
        self.config = config
        ...
        # self.inv_K = torch.linalg.inv(K_np).unsqueeze(0)
    ...
    def forward(self, prediction, gt, inv_K=None):
        # print('Im in %d now, plane forward 1' % torch.cuda.current_device())
        fit_prediction = self.surface_normal_from_depth(prediction, prediction>0)
        # print('Im in %d now, plane forward 2' % torch.cuda.current_device())
        fit_gt = self.surface_normal_from_depth(gt, gt>0)
        # print('Im in %d now, plane forward 3' % torch.cuda.current_device())
        abs_err = torch.abs(fit_prediction - fit_gt)
        # print('Im in %d now, plane forward 4' % torch.cuda.current_device())
        loss = torch.mean(abs_err)
        # print('Im in %d now, plane forward 5' % torch.cuda.current_device())
        return loss
