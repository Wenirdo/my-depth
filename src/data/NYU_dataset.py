...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = self.dataset_path + '/' + self.paths['image'][idx]
        depth_path = self.dataset_path + '/' + self.paths['depth'][idx]
        # norm_path = self.dataset_path + '/' + self.paths['norm'][idx]
        norm_path = 'configs/datasets/nyu_depth_v2/sync/bedroom_0086/norm_00036.png'
        ...
        norm_gt = norm_gt.crop((41, 45, 601, 471))
        ...
        if self.mode == 'train' and self.do_random_rotate is True:
            ...
            norm_gt = self.rotate_image(norm_gt, random_angle, flag=Image.NEAREST)
...
class Nyu2DataModule(LightningDataModule):
    def __init__(self, config: DictConfig):
        super().__init__()
        self.config = config
        # self.train_csv_path = os.path.join(config.DATASET.PATH, config.DATASET.TYPE, 'data', 'train.txt')
        # self.test_csv_path = os.path.join(config.DATASET.PATH, config.DATASET.TYPE, 'data', 'test.txt')
        # confusing original code? change to:
        self.train_csv_path = os.path.join(config.DATASET.PATH, 'nyu_train.txt')
        self.test_csv_path = os.path.join(config.DATASET.PATH, 'nyu_test.txt')
